...
for (let i = 0; i < lines.length; i++) {
  const match = lines[i].match(/{\s*id:\s*(\d+)/);
  if (match) {
    const id = parseInt(match[1]);
    if (socialData[id]) {
      // Remove everything from ', description: ' to the end of the line
      let baseStr = lines[i].replace(/, description:.*$/, '');
      if (baseStr.endsWith(' }')) {
        baseStr = baseStr.slice(0, -2);
      } else if (baseStr.endsWith('}')) {
        baseStr = baseStr.slice(0, -1);
      }
      lines[i] = baseStr + `, description: ${JSON.stringify(socialData[id].description)}, variants: ${JSON.stringify(socialData[id].variants)} }`;
    }
  }
}

fs.writeFileSync(dataFile, lines.join('\\n'), 'utf8');
console.log('Fixed premium-data.js with SPANISH descriptions and variants.');
